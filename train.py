...
class CONFIG:
    train_debug = False
    seed = 42
    train_path = 'DATA/processed_train.json'
    train_path_more = 'External Data/mixtral-8x7b-v1.json'
...
# Load Data from CSV    
data_original = pd.read_json(CONFIG.train_path)
data_extended = pd.read_json(CONFIG.train_path_more)
data_original.drop(columns=['document','trailing_whitespace','new_label_count','extra_label_new'],inplace=True)
data_extended.drop(columns=['document','full_text','trailing_whitespace'],inplace=True)
data = pd.concat([data_original, data_extended])

train_data, valid_data = train_test_split(data, test_size=0.2, random_state=CONFIG.seed)
